...
export function createKillerState(setup: KillerSetup): KillerGameState {
  ...
  const players: KillerPlayer[] = setup.players.map((p, idx) => ({
    ...
    name: p.name,
    color: p.playerColor,
    assignedNumber: "assignedNumber" in p && typeof (p as { assignedNumber?: number }).assignedNumber === "number"
      ? (p as { assignedNumber: number }).assignedNumber
      : idx + 1,
    armProgress: 0,
    isArmed: false,
...
export function applyKillerDart(
  ...
  if (segment !== null && segment === current.assignedNumber) {
    ...
    const requireMaxLivesToArm = state.settings.fullLivesToArm ?? false;
