...
export default function GolfGameScreen({ route, navigation }: Props) {
  ...
  const ui = useMemo(() => {
    ...
    return {
      ...
      boardSize: Math.min(screenWidth - 24, screenHeight - (isLarge ? 340 : 260), isLarge ? 950 : 520),
  ...
  const MIN_AUTO_DART_GAP_MS = 350;
  ...
  // =========================
  // AutoDarts: "connected" indicator (true if we've seen status recently)
  // =========================
  const [autoConnected, setAutoConnected] = useState(false);
  const autoConnTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  // =========================
  // Nassau Leg Tracker (labels) — MODE ONLY (independent of rewards)
  // =========================
  ...
  onPress={() => {
    // cancel any scheduled AUTO commit
    if (autoCommitTimerRef.current) {
      clearTimeout(autoCommitTimerRef.current);
      autoCommitTimerRef.current = null;
    }

    // ✅ clear overlay on undo (whether it's pending or state undo)
    setAutoOverlayText(null);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.full}>
        <View style={styles.topBar}>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            {/* LEFT: Title + subtitle (+ optional extra line) */}
            <View style={{ gap: 6, flex: 1 }}>
            ...
            <View style={styles.autoRightGroup}>
              ...
              <Pressable
                ...
                onPress={restartAutoDartsHard}
              >
                <Text style={styles.autoResetText}>Restart</Text>
              </Pressable>
  ...
  <View pointerEvents="none" style={styles.holeWatermarkWrap}>
    <Text
      style={[
        ...
        { fontSize: isLarge ? 150 : 32 }, // smaller than number
      ]}
    >
      {currentPlayer.name}
    </Text>
    ...
    <Text
      style={[
        ...
        { fontSize: isLarge ? 500 : 160 }, // big number
      ]}
    >
      {currentHole === "BULL" ? "B" : String(currentHole ?? "")}
    </Text>
  </View>
)}
      ...
      return (
        ...
        <View style={[styles.bottomBar, { paddingBottom: (isLarge ? 18 : 12) + (Platform.OS === "ios" ? 8 : 0) }]}>
          <View style={styles.buttonsRow}>
            ...
            <Pressable style={[styles.btn, { paddingVertical: ui.btnPadV }]} onPress={() => setState((s) => undo(s))}>
              <Text style={[styles.btnText, { fontSize: ui.btnText }]}>Undo</Text>
            </Pressable>

            <Pressable style={[styles.btn, { paddingVertical: ui.btnPadV }]} onPress={() => setState((s) => redo(s))}>
...
const styles = StyleSheet.create({
  ...
  exitLinkText: { color: "white", fontWeight: "900" },
...
holeWatermarkPlayer: {
  fontWeight: "900",
  color: "rgba(0,0,0,0.12)", // same transparency as number
  textAlign: "center",
  marginBottom: 10,
},
