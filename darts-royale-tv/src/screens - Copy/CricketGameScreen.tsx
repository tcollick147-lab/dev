...
const restartAutoDartsHard = useCallback(async () => {
  ...
  const rowH = clamp(
    Math.floor(usableLeaderH / Math.max(1, rowsN)),
    isLarge ? 56 : 48,
    isLarge ? 98 : 84
  );
              ...
              <Pressable
                ...
                onPress={() => {
                  ...
                  if (pendingRef.current.length > 0) {
                    ...
                    if (next.length > 0) showOverlayHold(pendingLabel(next[next.length - 1]));
                    else hideOverlay();
                  } else {
                    hideOverlay();
                    setState((s) => undo(s));
  ...
  onPress={() => {
    ...
    // ✅ DO NOT clear pending — this is just switching views
    setMode("LEADER");
  }}
>
          ...
          <View style={styles.topBarRow}>
            <View style={{ gap: 6 }}>
              ...
              <Text style={[styles.sub, { fontSize: ui.sub }]}>
                {winner.winnerIdx !== null
                  ? `🏁 Complete • Winner: ${playerName(state.players[winner.winnerIdx])}`
                  : `Tap anywhere to throw • Up next: ${playerName(state.players[state.turn])}`}
              </Text>
            </View>
...
>
  <Text style={styles.autoRestartText}>Restart</Text>
        ...
        >
          <View style={[styles.tableWrap, { padding: pad }]}>
        ...
        >
          ...
          {winner.winnerIdx === null && (
            <Text style={[styles.hint, { fontSize: isLarge ? 14 : 11 }]}>
              Tap anywhere on the scoreboard to throw. Use Undo/Redo for the last committed turn.
            </Text>
          )}
        </View>
      </View>
