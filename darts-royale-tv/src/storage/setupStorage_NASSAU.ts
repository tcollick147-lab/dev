...
const KEY_SETUP = "dartsroyale_setup_v2"; // setup schema key
...
export type PersistedSetup = {
  ...
  golf?: {
    ...
    side: {
      ...
      // ✅ resolve front-9 playoff after 9 holes
...
export async function loadSetup(): Promise<PersistedSetup | null> {
  try {
    ...
    if (golf && typeof golf === "object") {
      ...
      // If Nassau ON but flags missing, assume all legs ON.
      ...
      const nassauOverallOn =
        typeof golfSide.nassauOverallOn === "boolean"
          ? asBool(golfSide.nassauOverallOn)
          : nassauOn;
