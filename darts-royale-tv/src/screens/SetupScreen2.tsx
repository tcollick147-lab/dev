...
export default function SetupScreen({ navigation, route }: Props) {
  ...
  const [golfCellMode, setGolfCellMode] =
    useState<GolfCellMode>("HOLE");
  ...
  function addToMaster() {
    const next = `Player ${allPlayers.length + 1}`;
    setAllPlayers((p) => [...p, next]);
  }
  ...
  useEffect(() => {
    ...
    setGolfSide((s) => {
      const nextPlacement =
        s.placement.length === n
          ? s.placement
          : ensureDefaultPlacement(n);
      ...
      golf: {
        ...
        side: golfSide.enabled
          ? {
              ...
              nassauBack9Double: !!golfSide.nassauBack9Double,
              ...
              // ✅ Nassau timing
              nassauFrontResolveAt9: !!golfSide.resolveFront9After9,
          ...
          <View style={{ flex: 1 }}>
            ...
            <ScrollView style={styles.playerBox}>
              {allPlayers.map((name) => {
                ...
                return (
                  <View key={name} style={{ flexDirection: "row", gap: 8 }}>
                    <Pressable
                      style={[
                        styles.playerRow,
                        inGame && { opacity: 0.4 },
                    ...
                    <Pressable
                      style={[
                        styles.smallDanger,
                        inGame && styles.disabled,
                      ]}
                      disabled={inGame}
                      onPress={() => {
                        Alert.alert(
                          "Remove player",
                          `Remove "${name}" from All Players?`,
                          [
                    ...
                    >
                      <Text style={styles.smallDangerText}>🗑️</Text>
                    </Pressable>
                  </View>
                );
              })}
            ...
            <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
              <Pressable
                style={[
                  ...
                  !(allPlayers.length < 8) && styles.disabled,
                ]}
              ...
              {(["1_9", "1_18", "1_18_BULL"] as const).map((p) => {
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => setPresetSelection(p, setGolfSelected, setGolfBullOn)}
                  >
                    <Text
                      style={[
                        styles.pillText,
          ...
          <View style={[styles.card, isLarge && stylesL.card]}>
            ...
            <View style={[styles.row, { marginTop: 16 }]}>
              ...
              <Switch
                ...
                onValueChange={(v) => setGolfSide((s) => ({ ...s, nassauOn: v }))}
                disabled={!golfSide.enabled || !golfNassau || !nassauValid}
              />
            </View>

            <Text style={[styles.hint, isLarge && stylesL.hint]}>
              Available only in Nassau mode with 18 holes + Bull. Uses the Podium table for each leg.
            </Text>

            <View style={[styles.row, { marginTop: 6 }]}>
              <Text style={[styles.toggleLabel, isLarge && stylesL.toggleLabel]}>
                Back 9 Double Rewards
              </Text>
              <Switch
                value={golfSide.nassauBack9Double}
                onValueChange={(v) =>
                  setGolfSide((s) => ({ ...s, nassauBack9Double: v }))
                }
                disabled={
                  !golfSide.enabled || !golfNassau || !nassauValid || !golfSide.nassauOn
                }
              />
            </View>
            ...
            <View style={[styles.row, { marginTop: 6 }]}>
              <Text style={[styles.toggleLabel, isLarge && stylesL.toggleLabel]}>
                Resolve Front 9 after 9 holes
              </Text>
              <Switch
                value={golfSide.resolveFront9After9}
                onValueChange={(v) =>
                  setGolfSide((s) => ({ ...s, resolveFront9After9: v }))
                }
                disabled={
                  !golfSide.enabled || !golfNassau || !nassauValid || !golfSide.nassauOn
                }
            ...
            {/* (Eagle streak + Round score blocks continue in Part 3) */}
          ...
          <View style={[styles.card, isLarge && stylesL.card]}>
            ...
            <View style={styles.row}>
              ...
              <Switch
                ...
                onValueChange={(v) =>
                  setSide((s) => ({ ...s, checkoutJackpotOn: v }))
                }
              />
            </View>
            <View style={styles.row}>
