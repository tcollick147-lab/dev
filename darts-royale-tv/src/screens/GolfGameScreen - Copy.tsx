...
{!state.isComplete && (
  <View pointerEvents="none" style={styles.holeWatermarkWrap}>
    <View style={styles.holeWatermarkPlayerRow}>
      ...
      >
        {scoreLabel(currentPlayer.total)}
        {showFrontBackInTotalRow && front9Totals && back9Totals && currentPlayer.holeIndex >= 10 && (() => {
          const back = back9Totals[state.currentPlayerIndex] ?? 0;
          return (
            <> ({scoreLabel(front9Totals[state.currentPlayerIndex] ?? 0)}{back === 0 ? "+0" : scoreLabel(back)})</>
          );
        })()}
      </Text>
    </View>
