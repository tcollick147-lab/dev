...
// - EAGLE: an eagle that is currently part of an active streak (provisional until payout)
// - BONUS: bonus payout hole
// - JACKPOT: jackpot payout hole
...
export type GolfSide = {
  ...
  placementOn: boolean;
  placement: number[]; // 1st..Nth payouts, length = playerCount, sum = 0 when enabled
  ...
  /**
   * ✅ NEW: allow front-9 playoff/tie resolution as soon as front-9 is complete (after 9 holes),
   * rather than only at game end. (UI can prompt immediately when this becomes true)
   */
  nassauFrontResolveAt9: boolean;
...
// We keep your existing order fields, but add *Mode* for each.
// - If mode is null, it means "not resolved yet"
// - If mode is "TIE", order may remain null
// - If mode is "PLAYOFF", order is expected to be provided (full order OR tied-group order)
...
// ---------- Creation ----------
export function createGolfState(opts: {
  playerNames: string[];
  holes?: Hole[];
  side?: Partial<GolfSide>;
}): GolfState {
  ...
  const fallbackPlacement = Array.from({ length: playerCount }, (_, i) =>
    i === 0 ? 10 : i === playerCount - 1 ? -10 : 0
  );
  ...
  const marks: EagleRewardMark[][] = Array.from({ length: playerCount }, () =>
...
function normalizeSide(base: GolfSide, playerCount: number, patch?: Partial<GolfSide>): GolfSide {
  ...
  // ✅ Rule: Nassau and Podium cannot both be ON.
  if (merged.nassauOn) merged.placementOn = false;
  ...
  merged.nassauFrontResolveAt9 = !!merged.nassauFrontResolveAt9;
...
export function evalDart(hole: Hole, dart: GolfDart): Eval {
  if (hole === "BULL") {
    if (dart.kind === "DB")
      return { score: -2, label: "Red Bull (Eagle)", isEagle: true, isDoubleBogey: false };
    if (dart.kind === "SB")
      return { score: 0, label: "Single Bull (Par)", isEagle: false, isDoubleBogey: false };
    return { score: 2, label: "No Score (Double Bogey)", isEagle: false, isDoubleBogey: true };
  }

  if (dart.kind === "D" && dart.n === hole)
    return { score: -2, label: "Double (Eagle)", isEagle: true, isDoubleBogey: false };
  if (dart.kind === "T" && dart.n === hole)
    return { score: -1, label: "Treble (Birdie)", isEagle: false, isDoubleBogey: false };
  if (dart.kind === "S_IN" && dart.n === hole)
    return { score: 0, label: "Single Inner (Par)", isEagle: false, isDoubleBogey: false };
  if (dart.kind === "S_OUT" && dart.n === hole)
    return { score: 1, label: "Single Outer (Bogey)", isEagle: false, isDoubleBogey: false };
...
function normalizePlacementToPlayerCount(raw: number[], playerCount: number) {
  const placement = Array.from({ length: playerCount }, (_, i) => Number(raw?.[i] ?? 0));
  const sum = placement.reduce((a, b) => a + b, 0);
  if (placement.length) placement[placement.length - 1] -= sum; // force sum 0
  return placement;
}
...
function hasAnyTie(totals: number[]) {
  const seen = new Set<number>();
...
/**
 * Tie split logic for ONE group (positions [start,end) share those placement slots equally)
 * Returns { ok, deltas, eachShare } where eachShare is what each tied player gets.
 */
function tieSplitForGroup(
  ...
  // Must split evenly as an integer
  if (payoutSum % k !== 0) {
  ...
  // Must be divisible by divisor (default 5)
  ...
  if (each % d !== 0) {
    return {
      ...
      deltas,
      eachShare: each,
      reason: `Each share must be divisible by ${d}`,
    };
  }
...
/**
 * Apply ties across ALL groups (fallback old behaviour).
 * Note: this can create fractions; we keep it for “no playoff” fallback when nothing else is possible.
 */
function computePlacementDeltasFromTotals(totals: number[], placement: number[]) {
  ...
  while (pos < n) {
    ...
    let payoutSum = 0;
    for (let j = 0; j < k; j++) payoutSum += placement[start + j] ?? 0;

    const each = k ? payoutSum / k : 0;
    for (const playerIdx of group) deltas[playerIdx] = each;
  }
...
function getResolution(s: GolfState, kind: PlayoffKind): { mode: PlayoffMode | null; order: number[] | null } {
  ...
  if (kind === "NASSAU_FRONT")
    return { mode: s.playoffs.nassauFrontMode ?? null, order: s.playoffs.nassauFrontOrder ?? null };
  if (kind === "NASSAU_BACK")
    return { mode: s.playoffs.nassauBackMode ?? null, order: s.playoffs.nassauBackOrder ?? null };
 ...
 * and any multiplier (Nassau back-9 can be doubled).
 */
function getTieGroupForKind(s: GolfState, kind: PlayoffKind): { group: TieGroup | null; placement: number[]; multiplier: number } {
  const n = s.players.length;
  const placement = normalizePlacementToPlayerCount(s.side.placement ?? [], n);

  if (kind === "PODIUM") {
    const totals = s.players.map((p) => p.total);
    return { group: firstTieGroupFromTotals(totals), placement, multiplier: 1 };
  }

  if (!(s.side?.nassauOn && isNassauRound(s.holes))) return { group: null, placement, multiplier: 1 };
  ...
  if (kind === "NASSAU_FRONT") {
    if (!legComplete(s.players, frontIdxs)) return { group: null, placement, multiplier: 1 };
    const totals = legTotals(s.players, frontIdxs);
    return { group: firstTieGroupFromTotals(totals), placement, multiplier: 1 };
  }

  if (kind === "NASSAU_BACK") {
    if (!legComplete(s.players, backIdxs)) return { group: null, placement, multiplier: s.side.nassauBack9Double ? 2 : 1 };
    const totals = legTotals(s.players, backIdxs);
    return { group: firstTieGroupFromTotals(totals), placement, multiplier: s.side.nassauBack9Double ? 2 : 1 };
  }

  // overall
  if (!legComplete(s.players, overallIdxs)) return { group: null, placement, multiplier: 1 };
 ...
 * This is what allows you to only ask the tied players.
 */
function expandTiedOrderToFullOrder(totals: number[], tiedGroup: TieGroup, tiedOrder: number[]) {
  ...
  // Replace the tie slice with the provided order
...
export function getPlayoffNeeds(s: GolfState): PlayoffNeeds {
  ...
  // Podium: only when enabled, and Nassau not using it
  const podiumPossible = !!s.side?.enabled && !!s.side?.placementOn && !s.side?.nassauOn;
  const podiumGroup = podiumPossible ? getTieGroupForKind(s, "PODIUM").group : null;
  const podiumTie = !!podiumGroup;
  ...
  if (s.side?.enabled && s.side?.nassauOn && isNassauRound(s.holes)) {
    ...
    // ✅ front can be resolved early if configured, otherwise only require at game end
    frontTie = !!front && (s.isComplete || !!s.side.nassauFrontResolveAt9);
    backTie = !!back && s.isComplete;
    overallTie = !!overall && s.isComplete;
  }

  // Needs only if tie exists AND we can't auto-resolve via tie-only mode
  const needsKind = (kind: PlayoffKind, exists: boolean) => {
    ...
    if (s.side.tiesAllowed && !s.side.playoffsAllowed) {
      const chk = tieSplitForGroup(s.players.length, placement, group, s.side.tieDivisor, multiplier);
      return !chk.ok; // needs help only if tie split is not possible
    }

    // Otherwise we need a popup (because either playoff is allowed or both are allowed)
    return true;
  };

  return {
    podium: needsKind("PODIUM", podiumTie),
...
/**
 * ✅ NEW: ask engine what options should appear in the popup for a given kind.
 * UI uses this to decide whether to offer:
 * - “Keep Tie (split)” and/or
 * - “Playoff”
 */
...
/**
 * ✅ NEW: choose tie mode (split) explicitly.
 * If tie-only mode is on, UI may never call this — engine will auto-resolve — but it’s useful
 * when both modes are allowed and the user picks “Keep Tie”.
 */
 ...
 *
 * Supports BOTH:
 * 1) old behaviour: full permutation length = n
 * 2) new behaviour: tied-group order length = tiedGroupSize (only tied players)
 */
export function setPlayoffOrder(s: GolfState, kind: PlayoffKind, order: number[]): GolfState {
  const s2 = pushHistory(s);
  const n = s2.players.length;

  // Full permutation (legacy)
  ...
  // Tied-group order
  const { group } = getTieGroupForKind(s2, kind);
  if (!group) return s2;
  ...
  // Expand to full order using totals
  const totals =
    kind === "PODIUM"
      ? s2.players.map((p) => p.total)
      : getTotalsForKindOrNull(s2, kind) ?? s2.players.map((p) => p.total);
...
// helper: get totals array for Nassau legs
function getTotalsForKindOrNull(s: GolfState, kind: PlayoffKind): number[] | null {
  if (!(s.side?.nassauOn && isNassauRound(s.holes))) return null;
  const idx = holeIndexMap(s.holes);
  ...
  if (kind === "NASSAU_FRONT") return legTotals(s.players, frontIdxs);
  if (kind === "NASSAU_BACK") return legTotals(s.players, backIdxs);
  if (kind === "NASSAU_OVERALL") return legTotals(s.players, overallIdxs);
  return null;
}
...
function recomputeAllRewards(s: GolfState): GolfState {
  ...
  // keep placement normalized
  s.side.placement = normalizePlacementToPlayerCount(s.side.placement ?? [], n);

  // ✅ safety: Nassau and Podium can't both be ON
  if (s.side.nassauOn) s.side.placementOn = false;

  // 7.1 Podium
  let podium = [...zero];
  if (s.side.placementOn && !s.side.nassauOn) {
    const totals = s.players.map((p) => p.total);
    ...
    if (!group) {
      ...
      if (!res.mode && s.side.tiesAllowed && !s.side.playoffsAllowed) {
        const chk = tieSplitForGroup(n, s.side.placement, group, s.side.tieDivisor, 1);
        if (chk.ok) {
          podium = chk.deltas;
          s.playoffs.podiumMode = "TIE";
          s.playoffs.podiumOrder = null;
        } else {
          // cannot tie-split => hold until UI resolves (but playoff must be enabled in setup)
          podium = [...zero];
        }
      } else if (res.mode === "TIE") {
        const chk = tieSplitForGroup(n, s.side.placement, group, s.side.tieDivisor, 1);
        podium = chk.ok ? chk.deltas : [...zero];
      } else if (res.mode === "PLAYOFF") {
        if (isPermutation(res.order, n)) {
          podium = computeDeltasFromOrder(res.order as number[], s.side.placement);
        } else {
          podium = [...zero];
        }
      } else {
        // both modes allowed -> wait for user choice
        podium = [...zero];
      }
    }
  }
  ...
  if (s.side.nassauOn && isNassauRound(s.holes)) {
    ...
    // FRONT
    if (legComplete(s.players, frontIdxs)) {
      const totals = legTotals(s.players, frontIdxs);
      const group = firstTieGroupFromTotals(totals);
      ...
      if (!group) {
        ...
        const shouldResolveNow = s.isComplete || !!s.side.nassauFrontResolveAt9;

        if (shouldResolveNow) {
          if (!res.mode && s.side.tiesAllowed && !s.side.playoffsAllowed) {
            const chk = tieSplitForGroup(n, placement, group, s.side.tieDivisor, 1);
            if (chk.ok) {
              add(nassau, chk.deltas);
            ...
            const chk = tieSplitForGroup(n, placement, group, s.side.tieDivisor, 1);
            if (chk.ok) add(nassau, chk.deltas);
          } else if (res.mode === "PLAYOFF") {
            if (isPermutation(res.order, n)) add(nassau, computeDeltasFromOrder(res.order as number[], placement));
          } else {
            // wait
          }
        } else {
          // not time yet (no payout)
        }
      }
    }

    // BACK (only resolves at end)
    if (legComplete(s.players, backIdxs) && s.isComplete) {
      const totals = legTotals(s.players, backIdxs);
      const group = firstTieGroupFromTotals(totals);
      const times = s.side.nassauBack9Double ? 2 : 1;

      const addTimes = (d: number[]) => {
        for (let t = 0; t < times; t++) add(nassau, d);
      };

      if (!group) {
        addTimes(computePlacementDeltasFromTotals(totals, placement));
        ...
        if (!res.mode && s.side.tiesAllowed && !s.side.playoffsAllowed) {
          const chk = tieSplitForGroup(n, placement, group, s.side.tieDivisor, times);
          if (chk.ok) {
            addTimes(chk.deltas);
          ...
          const chk = tieSplitForGroup(n, placement, group, s.side.tieDivisor, times);
          if (chk.ok) addTimes(chk.deltas);
        } else if (res.mode === "PLAYOFF") {
          if (isPermutation(res.order, n)) addTimes(computeDeltasFromOrder(res.order as number[], placement));
        } else {
          // wait
        }
      }
    ...
    if (legComplete(s.players, overallIdxs) && s.isComplete) {
      const totals = legTotals(s.players, overallIdxs);
      const group = firstTieGroupFromTotals(totals);
